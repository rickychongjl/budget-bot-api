...
import { LlmParseError, type LlmParseResult, type LlmParser, type LlmParserContext, type LlmUsage } from '../core/ports/llm-parser';
import type { Logger } from '../observability/log';
import { NoopLogger } from '../observability/log';

/**
 * Stage 6 (M6 "End-to-end flow"): the schema-constrained LLM parser behind M1's
 ...
 */
export type ILlmTransactionParser = LlmParser;
...
export interface OpenAiLlmTransactionParserOptions {
  apiKey: string;
  clock: Clock;
  logger?: Logger;
  model?: string;
...
export class OpenAiLlmTransactionParser implements ILlmTransactionParser {
  private readonly client: OpenAI;
  private readonly clock: Clock;
  private readonly logger: Logger;
  private readonly model: string;

  constructor(options: OpenAiLlmTransactionParserOptions) {
