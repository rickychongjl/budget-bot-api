...
- **No slash command counts toward the Free daily cap — Ricky, 13 Sep 2026.** This reverses part of the 11 Sep ruling's scope: that one waived the cap only until onboarding finished, and left every command except `/help`, `/subscription` and `/paysupport` counting, so a Free user who spent their five messages was refused `DAILY_MESSAGE_LIMIT` on `/budget`, `/categories`, `/delete`, `/today`, `/history`, `/stats`, `/remind`, `/settings` and `/cancel` — locked out of managing the account, including out of correcting the entries that used up the day. The reasoning is the 11 Sep one applied more widely: **the daily cap is a limit on using the product (free-text expense logging), not a toll on running the account.** Two things it does *not* change: **fair use (20 / rolling 2h) still applies to commands** and can still refuse one with `FAIR_USE_LIMIT` — abuse protection is never waived; and an *unrecognised* `/whatever` still counts, so a leading slash is not a free pass. `exemptFromAdmission` (`/help`, `/subscription`, `/paysupport`) keeps its separate, stronger meaning: those three skip the rate limiter entirely.
...
| Free text | Log an expense or income in plain language. Records outright, asks one confirmation, or asks one clarification — **shipped 13 Sep 2026** (M7 stage 4D). Yes/no questions carry an inline keyboard *and* accept a typed yes/no; anything else supersedes them | Both; shipped; **the only thing that counts toward the daily cap**; fair use | M6 → M3 |
| `/today [category]` | Today's allowance | Both; fair use only, no daily cap (13 Sep) | M5, M8 |
