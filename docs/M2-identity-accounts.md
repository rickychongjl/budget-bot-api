...
5. Implement settings mutation rules:
   ...
   **No command surfaces these this pass — Ricky's call, 11 Sep 2026 (M7 stage 4C).** `updateSettings` keeps its contract and every rule above; `/settings` is a **read-only** view. Between the immutable timezone, the currency refused after a first transaction, and the fixed 07:00 reminder, the budget anchor date was the only genuinely editable field, and shipping the read while deferring the write beat building an edit path for one field. **Accepted cost: a budget start date mistyped during onboarding cannot be corrected without deleting the account** — the thing to revisit first whenever settings editing comes back into scope. M7 enforces this structurally rather than by convention: its `CommandServices.identity` is `Pick<IdentityService, 'getSettings' | 'register'>`, so a handler that tried to write would not compile.
