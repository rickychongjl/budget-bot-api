...
`daily_target` is computed once per category per date, written, and **never recomputed from that day's spend**. If it were recomputed live from current spend, overspending at lunch would immediately spread across the remaining days, `available_today` would quietly stay positive, and the user would never see they'd gone over. Persisting means today has a fixed budget: going over is visible today as a negative number, and the correction arrives tomorrow from a genuinely smaller remaining balance. **Recalculation on a transaction change touches `available_today` only — never `daily_target`.**

**The one exception — a cap change (Ricky, 11 Sep 2026).** `/budget groceries 600` mid-cycle is meant to change what the user can spend *today*, not from tomorrow. M4 calls `capChanged(userId, categoryId)` after its writes, and M5 rewrites today's row in place: the new period cap, spend **to the end of yesterday**, days left including today — the same inputs as the morning compute, so it is a pure function of state that does not move during the day and cannot reintroduce the smear above. Delivery state is untouched: a row the 07:00 bundle already sent stays `sent`, nothing is re-sent, and the morning message is simply history. Today's own spend is still measured *against* the re-priced target, so an overspend stays visible. `updateTarget` on the repository exists for this and nothing else.
